    ...
    Parameters(const ros::NodeHandle &nh_priv) {
        auto requireParam = [&](const std::string& name, auto& value) {
            if (!nh_priv.getParam(name, value)) {
                const std::string msg = "nanovoxmap: required parameter '" + name + "' is missing";
        ...
        if (!(VoxelResolution > 0.0)) {
            const std::string msg = "nanovoxmap: VoxelResolution must be > 0, got " + std::to_string(VoxelResolution);
            ROS_FATAL_STREAM(msg);
            throw std::runtime_error(msg);
        }
        ...
        if (!(EsdfMaxDistance > 0.0)) {
            const std::string msg = "nanovoxmap: EsdfMaxDistance must be > 0, got " +
                                    std::to_string(EsdfMaxDistance);
            ROS_FATAL_STREAM(msg);
            throw std::runtime_error(msg);
        }
    }
...
class NanoVoxMapNode {
    ...
    private:
        ...
        /// Owned by esdfTimerCallback() alone, so it needs no extra locking.
