...
public:
    ...
    explicit OccupancyMap(Scalar resolution,
        ...
        occupied_indices_.reserve(4096);
        occupied_indices_.max_load_factor(0.7f);
    ...
    void snapshotOccupied(std::vector<Idx>& out,
        ...
        out.reserve(occupied_indices_.size());
        for (const GridIndex& v : occupied_indices_) {
    ...
    std::vector<Vec3> getOccupiedVoxels() const {
        ...
        result.reserve(occupied_indices_.size());
        for (const GridIndex& idx : occupied_indices_) result.push_back(gridToWorld(idx));
        return result;
    }

    size_t numOccupied() const { return occupied_indices_.size(); }
    ...
    Fingerprint fingerprint() const {
        ...
        for (const auto& item : blocks_) {
            ...
            const VoxelBlock& block = item.second;
    ...
    size_t allocatedBytes() const { return blocks_.size() * sizeof(VoxelBlock); }
    ...
    void computeEsdf() const {
        std::vector<GridIndex> sites(occupied_indices_.begin(), occupied_indices_.end());
        esdf_nodes_.clear();
    ...
    void rebuildEsdf() const {
        ...
        seeds.reserve(occupied_indices_.size() / 8 + 1);
        for (const GridIndex& voxel : occupied_indices_) seeds.insert(blockIndex(voxel));
...
private:
    ...
    static constexpr float kEsdfInf = 1e18f;
    ...
    std::unordered_map<BlockIndex, VoxelBlock, IndexHash> blocks_;
    std::unordered_set<GridIndex, IndexHash> occupied_indices_;
     ...
     * either backend can serve any box.  Returns false if the device could not
     * take the work, in which case the caller runs the CPU solver instead.
     */
    bool solveSignedEsdfCuda(const std::vector<uint8_t>& occupancy,
    ...
    static int localCoordinate(int voxel, int /*block*/) {
    ...
    int8_t& voxelRef(const GridIndex& voxel) {
        ...
        VoxelBlock& data = blocks_[block];
        return data.voxels[localOffset(localCoordinate(voxel.x, block.x),
                                       localCoordinate(voxel.y, block.y),
                                       localCoordinate(voxel.z, block.z))];
    }
    ...
    int8_t& voxelRefCached(BlockCursor& cursor, const GridIndex& voxel) {
        ...
        if (cursor.data == nullptr || !(cursor.block == block)) {
            cursor.data = &blocks_[block];
            cursor.block = block;
        }
        return cursor.data->voxels[localOffset(localCoordinate(voxel.x, block.x),
                                               localCoordinate(voxel.y, block.y),
                                               localCoordinate(voxel.z, block.z))];
    }
    ...
    int8_t voxelValue(const GridIndex& voxel) const {
        ...
        return found->second.voxels[localOffset(localCoordinate(voxel.x, block.x),
                                                localCoordinate(voxel.y, block.y),
                                                localCoordinate(voxel.z, block.z))];
    }
    ...
    void updateOccupiedSet(const GridIndex& voxel, int8_t before, int8_t after) {
        if (before <= 0 && after > 0) {
            occupied_indices_.insert(voxel);
            esdf_dirty_.insert(blockIndex(voxel));
        } else if (before > 0 && after <= 0) {
            occupied_indices_.erase(voxel);
            esdf_dirty_.insert(blockIndex(voxel));
        }
    }
    ...
    float esdfValueAtVoxel(const GridIndex& voxel, bool& materialized) const {
        ...
        materialized = true;
        return found->second.dist[localOffset(localCoordinate(voxel.x, 0),
                                              localCoordinate(voxel.y, 0),
                                              localCoordinate(voxel.z, 0))];
    }
    ...
    void solveEsdfRegion(const BlockBox& region) const {
        ...
        for (Idx bz = blo[2]; bz <= bhi[2]; ++bz)
            for (Idx by = blo[1]; by <= bhi[1]; ++by)
                for (Idx bx = blo[0]; bx <= bhi[0]; ++bx) {
                    ...
                    const VoxelBlock& block = found->second;
                    for (int lz = 0; lz < kVoxelsPerSide; ++lz) {
                        ...
                        for (int ly = 0; ly < kVoxelsPerSide; ++ly) {
                            ...
                            for (int lx = 0; lx < kVoxelsPerSide; ++lx) {
                                if (block.voxels[localOffset(lx, ly, lz)] <= 0) continue;
                                const Idx x = bx * kVoxelsPerSide + lx - slo[0];
                                if (x < 0 || x >= n[0]) continue;
        ...
        if (!esdf_cuda_unavailable_) {
            solved = solveSignedEsdfCuda(occupancy, n[0], n[1], n[2], band, field);
            if (!solved) esdf_cuda_unavailable_ = true;
        }
#endif
    ...
    void occupiedBounds(GridIndex& low, GridIndex& high) const {
        const auto first = occupied_indices_.begin();
        low = high = *first;
        for (const GridIndex& voxel : occupied_indices_) {
    ...
    Scalar noObstacleDistance() const {
        return static_cast<Scalar>(std::sqrt(kEsdfInf)) * resolution_;
    }
};
